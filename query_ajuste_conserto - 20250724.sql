DECLARE @CHAVE AS VARCHAR(44) = '33250751372842000105550010000036581634253709'
...
SELECT
	...
	ISNULL(TF1.DIA_ENTRADA_LOJA, TF2.DIA_ENTRADA_LOJA) AS DIA_ENTRADA_LOJA,

	CASE 
		WHEN DAY(ISNULL(TF1.DIA_ENTRADA_LOJA, TF2.DIA_ENTRADA_LOJA)) BETWEEN 1 AND 7 THEN 1
		WHEN DAY(ISNULL(TF1.DIA_ENTRADA_LOJA, TF2.DIA_ENTRADA_LOJA)) BETWEEN 8 AND 15 THEN 2
		WHEN DAY(ISNULL(TF1.DIA_ENTRADA_LOJA, TF2.DIA_ENTRADA_LOJA)) BETWEEN 16 AND 23 THEN 3
		ELSE 4
	END AS NUMERO_SEMANA,
...
OUTER APPLY (
	...
	FROM (
		...
		FROM TEMP_COD TC
		WHERE TC.ORDEM_PRODUCAO = A.NUMERO_DOCUMENTO
...
OUTER APPLY (
	...
	WHERE PRODUTO = A.PRODUTO
	  AND COR_PRODUTO = A.COR
	  AND QTDE_ENTREGUE > 0
	  AND CANAL NOT LIKE '%MOSTRUARIO%'
	ORDER BY DIA_ENTRADA_LOJA DESC
) AS TF2
